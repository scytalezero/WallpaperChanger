...
namespace WallpaperChanger {
  ...
  class Wallpaper {
    ...
    public static int Set(String file, Style style, String storagePath) {
      ...
      catch (OutOfMemoryException ex) {  // thrown when the file does not have a valid image format or the decoder does not support the pixel format of the file
        Console.WriteLine("\nInvalid file format or the file format is not supported");
        return 1;
      }
      catch (Exception ex) { // catch everything else just in case
    ...
    static int Main(string[] args) {
      ...
      if (Wallpaper.Set(file, style, storagePath) == 1)
        return 1;
